...
var getHtmlTpl = function getHtmlTpl(html) {
  if (!html.includes("template")) {
    return "<div id=\"app\"><template>".concat(html, "</template></div>");
  }

  return "<div id=\"app\">".concat(html, "</div>");
};
...
var getVueScript = function getVueScript(js, html) {
  var scriptStrOrg = "(function() {; return  { data: () => ({ }),methods: {} }})()";

  if (js) {
    var scripts = js.split(/export\s+default/);
    scriptStrOrg = "(function() {".concat(scripts[0], " ; return ").concat(scripts[1], "})()");
  }
  ...
  scriptObj.template = "<div class=\"reaven-doc\">".concat(html, "</div>");
  return scriptObj;
};
...
var getVueDetail = function getVueDetail(code, config) {
  ...
  var htmlBlock = stripTemplate(code);
